...
- [x] **R07 — Verify output collisions and interruption messages.** Exercise existing run IDs/output paths, unreadable input, unwritable storage and Ctrl-C in supported long operations. **Done:** exact-head CI run `36158034367` on `ca57c212ec0bcb19eb07b7f2ce982c4599824604` passed quality, security, Python 3.11–3.14, packaging and the retained `tests/test_release_r07.py` qualification cases. Existing completed evidence survives run-ID collision; unreadable input returns an actionable controlled CLI error; write failure leaves a terminal `failed` manifest with the retained source snapshot; interruption before ledger publication rewrites the durable manifest to terminal `interrupted` state while preserving finalized artifacts.
