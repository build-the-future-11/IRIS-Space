...
known or suspected variable stars.** Those 55 objects are what an unfiltered pipeline
would have pushed to TNS as new transients. Rejecting them is the product.

A further 30 were rejected as objects already on TNS — real transients, but not I SPY's
to claim.

Moving-object rejections were zero throughout. The classifier pre-filter and freshness
scoring exclude Solar System objects before SkyBoT is reached; the check is retained as a
safety net, not a primary filter.
