...
Historical audit: the September 7 verification below describes an earlier source
state. Later evidence is recorded in `research/software-verification.json` and
the repository's `FINAL_TODO.md`. Synthetic search, covariance and noise-stress
experiments now exist under `experiments/`; these do not provide real-sky
completeness, prospective labels, live-service qualification or independent signoff.
The original adverse correlated-noise and isolated-outlier results are preserved.

Audit opened: 2026-09-06
...
The principal weakness was a mismatch between manuscript and repository maturity. The paper described the legacy I SPY scripts, while the repository already contained the newer SIDEREA architecture: canonical band-aware measurements, immutable scientific identities, evidence binding, deterministic queues, version-bound review, reporting preflight, and shadow-only learned representations. The revised paper describes the transition from I SPY to SIDEREA and keeps historical and current claims separate.
...
1. **Architecture drift:** the manuscript did not document the implemented SIDEREA evidence and review controls.
...
6. **Configuration ambiguity:** an appendix labeled legacy thresholds as current despite the checked-in SIDEREA policy having different radii, TTLs, review rules, and shadow-model controls.
...
- Retitled the paper **SIDEREA: Evidence-Bound and Human-Supervised Triage of Optical Transient Alerts** and identified IRIS as the conference rather than the software project.
...
- Rebuilt the SIDEREA architecture as coupled scientific-data, external-evidence, and human-decision planes, with interpretation left to the caption rather than embedded slogans.
...
- Added a current SIDEREA architecture figure with an explicit non-authoritative shadow-learning route.
...
- Split the appendix into final legacy thresholds and current SIDEREA safety defaults.
...
- Full suite: 320 tests and 96 parameterized subtests passed on each of Python 3.11, 3.13, and 3.14 with temporary files on the workspace volume.
...
- Deterministic replay: two executions of `examples/photometry.csv` produced the same scientific fingerprint (`analysis-d19729fbf526152c`), byte-identical normalized/features/ranking artifacts, and identical candidate versions.
...
The next defensible scientific extension is a frozen, object-grouped, time-forward replay cohort containing every eligible and rejected object. It must archive point-in-time broker state, raw or hashed external responses, image-review labels, policy version, review time, and matured outcomes. Only that cohort can support completeness, precision, gate ablation, learned-model comparison, and selection-function claims. Until then, SIDEREA should remain human supervised and all learned routes should remain shadow-only.
