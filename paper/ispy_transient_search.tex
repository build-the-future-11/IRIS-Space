...
\hypersetup{
  ...
  pdftitle={IRIS: An Auditable Pipeline for Human-Supervised Optical Transient Triage},
...
\renewcommand{\shorttitle}{IRIS: Auditable Optical Transient Triage}

\title{\iris: An Auditable Pipeline for Human-Supervised Optical Transient Triage}
...
\begin{abstract}
We present \iris, an auditable pipeline for human-supervised triage of optical transient alerts. The system was developed from an audit of \project, a small search program using public Zwicky Transient Facility alerts distributed through the ALeRCE broker. The historical pipeline ranked sources with interpretable light-curve features and then applied registry, catalog, moving-object, image, and report-wording checks. \iris\ retains this division of labor while adding band-aware measurements, content-addressed candidate versions, time-limited external evidence, version-bound review, and a deterministic allocation of finite review capacity. Reporting is governed by a conjunction of required clearances rather than by the ranking score; a missing, stale, malformed, or incorrectly bound result cannot satisfy the gate. The campaign record contains 24 productive runs between 2026 April and July and 3,824 distinct broker objects. After removal of one duplicated pull, 223,938 of 424,223 detection rows passed the recorded quality cuts. An undocumented policy change on July 5 coincided with a decrease in the early-veto fraction from 54.5\% to 2.68\% and an increase in the shortlist fraction from 0.51\% to 13.9\%. Run-cluster bootstrap intervals for the corresponding changes are $[-0.812,-0.282]$ and $[0.098,0.171]$. These differences describe pipeline operation and do not identify a causal effect. Of 200 recorded registry checks, 30 found an existing TNS object; 55 of 193 catalog evaluations found a known or suspected variable. Two submitted objects received TNS designations, including ZTF26abdqbqw = AT~2026rsp. At the audit freeze, the implementation passed 212 tests and 54 subtests, static analysis, and deterministic local replay. The available record supports claims about workflow behavior and tested software invariants, but not completeness, purity, or comparative classifier performance.
\end{abstract}
...
ZTF alert packets contain the current difference-image detection, historical photometry and upper limits, catalog context, and science, reference, and difference cutouts \citep{patterson2019}. The upstream ZTF Science Data System performs image subtraction and machine-learned reliability filtering before distribution \citep{masci2019,duev2019}. Consequently, the input to \project\ is already selected twice: first by ZTF alert production, then by ALeRCE ingestion and classification. Any \project\ selection function is conditional on both upstream layers.
...
\caption{Legacy \project\ workflow. Solid arrows show candidate advancement; dashed arrows show rejection, blocking, and registry outcomes. A negative query result is accepted only when the query completed successfully, so an unavailable required service cannot produce a false clearance.}
\label{fig:workflow}
\end{figure}

\section{IRIS Implementation}
\label{sec:iris}

The campaign used procedural controls that were not represented consistently in its saved run artifacts. In \iris, provenance, external checks, and review decisions are stored as typed records. Figure~\ref{fig:iris-architecture} summarizes the implementation. Measurements and configuration determine candidate identity; external-service results are validated and bound to that identity; and review and reporting operate on the resulting versioned dossier. The system accepts either a bounded broker snapshot or a local measurement table and does not include unattended registry submission.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{figures/iris_architecture.png}
\caption{Implemented \iris\ architecture as three coupled planes. Solid arrows carry authoritative state. External results are useful only after their identity, position, radius, epoch, lifetime, and response digest are bound to the candidate version. The violet shadow route may change review order but has no path around the red preflight gate.}
\label{fig:iris-architecture}
\end{figure}
...
We performed a local technical audit of the current \iris\ implementation on 2026 September 6 using Python 3.11.15. Table~\ref{tab:software-verification} distinguishes implementation evidence from scientific validation. With temporary SQLite files placed on the workspace volume, the audit-freeze suite completed 212 tests and 54 subtests without failure. The audit repaired an input-validation-order defect, strengthened per-epoch SkyBoT binding against displaced client responses, corrected a misplaced assertion and clock-dependent fixture, and resolved static-format issues. No score formula or scientific threshold changed.
...
\midrule
Unit/invariant suite & 212 tests + 54 subtests passed & Configuration, ingestion, photometry, scoring, clients, evidence binding, ledgers, review, baseline, JEPA, and command behavior. \\
Static analysis & Passed & \texttt{ruff} over \texttt{src}, \texttt{tests}, and the manuscript figure generator. \\
...
We have presented a run-level audit of the \project\ campaign and the implementation of its successor, \iris. The campaign processed 3,824 distinct ALeRCE objects in 24 productive runs. Among the recorded downstream evaluations, 30 of 200 registry checks identified an existing TNS object and 55 of 193 catalog checks identified a known or suspected variable. Two submitted sources received TNS designations; ZTF26abdqbqw = AT~2026rsp is the only case for which the repository contains a complete discovery record.
...
\section{Legacy and \iris\ Configuration}
...
\begin{table}[ht]
\centering
\caption{Checked-in \iris\ safety and allocation defaults used by the local technical audit. These settings define implementation behavior, not a scientifically validated operating point.}
\label{tab:iris-defaults}
