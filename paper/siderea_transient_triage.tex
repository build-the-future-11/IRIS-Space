...
\begin{abstract}
We present \siderea, an evidence-bound workflow for human-supervised triage of optical transient alerts. Its contribution is an explicit link between an immutable candidate version, the external evidence inspected, and the human decision permitted by that evidence. Rankings allocate review attention; missing, stale, malformed, or incorrectly bound checks cannot authorize report preparation. The design was motivated by an audit of the historical I SPY campaign, whose surviving record describes 24 productive runs and 3,824 distinct broker objects. Recorded registry and catalog evaluations identified 30 existing TNS objects among 200 checks and 55 known or suspected variables among 193 evaluations. These stage-specific counts describe operational filtering, not population purity or completeness. A documented policy discontinuity is analyzed descriptively because policy, time, and input population are confounded. Software tests and deterministic replay exercise the implemented evidence and review invariants. Separate synthetic template-search experiments demonstrate both conditional signal recovery and severe false-alarm inflation under misspecified noise. Historical outcomes, software verification, and synthetic experiments provide distinct forms of evidence; none establishes comparative detection performance on a representative sky sample. We specify the prospective cohort and independent outcome evidence needed to evaluate that stronger claim.
\end{abstract}
...
The primary contribution is a reproducible decision contract linking candidate identity, evidence freshness, and version-specific human approval. This paper evaluates that contract through software tests and uses the historical campaign to explain the failure modes it addresses; it does not claim a new validated astrophysical classifier. It reports the historical campaign and describes the implemented successor. Section~\ref{sec:related} places the work relative to alert brokers and follow-up systems. Sections~\ref{sec:data}--\ref{sec:methods} define the input data, score, decision states, and human review procedure. Section~\ref{sec:siderea} describes the current data and evidence model. Sections~\ref{sec:campaign}--\ref{sec:software-verification} report the campaign audit and local software verification. The available data do not support a completeness measurement, a false-negative rate, or a comparison with learned ranking methods; Section~\ref{sec:validation} specifies the prospective data needed for those analyses.
...
Most directly, \citet{rui2026} applies a domain-informed joint-embedding and multi-view self-distillation framework to irregular astronomical light curves, with evaluation on LEAVES, StarEmbed, and heterogeneous irregular-time-series benchmarks. The experimental JEPA route in \siderea is neither presented nor evaluated as an alternative state of the art: it is a smaller operational prototype whose distinctive role is restricted to queue allocation behind an independent reportability contract. Any future representation-learning claim will require a controlled comparison with that method and with masked-reconstruction and general-purpose time-series baselines.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.70\textheight,keepaspectratio]{figures/pipeline_flow.png}
...
Let $\mathcal{E}$ be the eligible candidates and $K$ the review budget. If the optional audit reserve is enabled, select $r=\min(r_{\rm requested},K,|\mathcal{E}|)$ candidates by a recorded seeded hash ordering, giving $\mathcal{R}$. Otherwise $\mathcal{R}=\emptyset$. Define $\mathcal{E}'=\mathcal{E}\setminus\mathcal{R}$, $K'=K-|\mathcal{R}|$, and $m'=\min(m,K')$ for requested anomaly reserve $m$. With threshold $\tau$, the remaining allocation is
\begin{equation}
\begin{aligned}
\mathcal{P} &= \operatorname{Top}_{K'-m'}(q_i:i\in\mathcal{E}'),\\
\mathcal{A} &= \operatorname{Top}_{m'}(a_i:i\in\mathcal{E}'\setminus\mathcal{P},\ a_i\geq\tau),\\
\mathcal{F} &= \operatorname{Top}_{K'-|\mathcal{P}|-|\mathcal{A}|}
(q_i:i\in\mathcal{E}'\setminus(\mathcal{P}\cup\mathcal{A})),\\
\mathcal{Q}_K &= \mathcal{R}\cup\mathcal{P}\cup\mathcal{A}\cup\mathcal{F}.
\end{aligned}
\label{eq:queue-allocation}
\end{equation}
Each $\operatorname{Top}_n$ returns at most $n$ available candidates. Priority ordering uses decreasing $q_i$, then decreasing $a_i$; anomaly ordering reverses those two keys, and both finish with a stable identifier tie-break. The fill set $\mathcal{F}$ returns unused anomaly slots to priority review. The audit sample is drawn from the full eligible population before either ranked route; its seed and inclusion probability $r/|\mathcal{E}|$ are recorded when applicable. The audit reserve is zero by default. Here $a_i$ may derive from engineered features or a learned embedding; none of these routes authorizes reporting.

The experimental irregular-time joint-embedding predictive architecture masks contiguous temporal windows and predicts their latent representations from the remaining context. The checked research profile samples short, nominal, and long target fractions around a declared base fraction. Its target encoder follows the context encoder with a cosine schedule from a responsive initial exponential-moving-average momentum to a more stable final momentum. The exact schedule and applied step count are checkpointed. Normalization is recomputed from unmasked context so hidden target values cannot determine the context scale, and streaming covariance diagnostics expose amplitude and low-rank collapse. This route is shadow-only: $a_i$ can affect membership in $\mathcal{Q}_K$, but it does not occur in Equation~\ref{eq:preflight-predicate}. These mechanisms are implementation safeguards and testable hypotheses; no result in this paper establishes that the embeddings improve astrophysical discovery.
...
Summary-recorded designated objects & 2 \\
Object with detailed surviving narrative & 1 \\
\bottomrule
\end{tabular}
\end{table}
...
The surviving operations summary records two submissions assigned TNS designations, AT~2026pcz and AT~2026rsp. The public TNS record independently verifies AT~2026rsp, including report 312644 and its ZTF26abdqbqw identity; AT~2026pcz remains summary-attributed. Relative to 3,824 distinct screened objects, the summary-attributed submitted-and-designated fraction is
\begin{equation}
  f_{\rm designated} = \frac{2}{3824} = 5.23\times10^{-4} = 0.052\%.
\end{equation}
Only AT~2026rsp has a detailed surviving narrative in the present repository. The fraction with that level of summary documentation, which is not an independently verified discovery yield, is
\begin{equation}
  f_{\rm narrative} = \frac{1}{3824} = 2.62\times10^{-4} = 0.026\%.
\end{equation}
The Wilson intervals are 0.014--0.191\% for the submitted-and-designated fraction and 0.0046--0.148\% for the single detailed summary case. These are operational yields rather than estimates of an astrophysical event rate or classifier precision. The denominator was created by a changing, targeted query and is not a controlled sample of the alert stream. Moreover, ``detailed surviving summary'' measures present-day provenance, not necessarily all historical scientific merit.
...
The public TNS object page independently verifies ZTF26abdqbqw as AT~2026rsp, object ID 212592, and identifies Aadi Ajeesh Nair as reporter on AT report 312644. It records discovery on 2026 June 19 10:11:11 UTC at $g=21.067\pm0.311$ AB mag. The fuller time-series narrative below remains attributed to the surviving operations summary because the original row-level cohort and submission response bytes are absent. That summary reports twenty-three positive detections in $g$, $r$, and $i$ over 14 nights through 2026 July 3, 36 pre-discovery non-detections, and a best limiting magnitude of 20.551. It reports brightening of approximately 1.3 mag in ten days and a peak near $r=19.8$ around 2026 July 1--4.

The position is associated with WISEA J222235.91+044418.8, and the ALeRCE light-curve classifiers favored a Type Ia supernova interpretation. The report nevertheless used unclassified-transient language because no spectrum was available. TNS verifies the designation, report identity, reporter, discovery datum, and lack of a classification report in the captured page. The AstroNote and fuller light-curve history remain summary-attributed. The summary states that the TNS report predates creation of reporting group 204 on July 9 and lists no reporting group; this historical attribution issue should not be propagated to future submissions.
...
The original frozen verification snapshot in \texttt{paper/research/software-verification.json} was executed on 2026 September 8 using Python 3.11 and remains available for historical traceability. A later clean-tree release verification, summarized in \texttt{paper/research/software-verification-2026-09-13.json}, binds revision \texttt{4e084a795be3} and the source digest to its logs and reconstructed paper inputs. It completed 499 tests and 103 parameterized subtests without failure, applied strict typing to 77 source files, and measured 77.95\% branch-aware coverage against a 70\% threshold. Table~\ref{tab:software-verification} distinguishes this implementation evidence from scientific validation. Neither snapshot is a moving claim about later repository revisions.
...
\midrule
Unit/invariant suite & 499 tests + 103 subtests passed & Configuration, ingestion, photometry, scoring, clients, evidence binding, ledgers, review, baseline, JEPA, and command behavior. \\
Static analysis & Passed & Formatting, lint, compilation, and strict typing over 77 source files. \\
Coverage & 77.95\% & Branch-aware coverage exceeded the frozen 70\% release threshold. \\
...
End-to-end shadow fixture & Completed & Preparation through final queue ran with atomic stage receipts; a controlled malformed-input run stopped and recorded failure at JEPA training. \\
\bottomrule
\end{tabular}
\end{table}
...
Tiny synthetic execution checks also exercised the logistic baseline and JEPA training paths. The baseline retained an uncalibrated-score label because its three-row calibration partition could not supply the required five examples per class. A three-epoch JEPA run and repeated-mask evaluation exercise checkpointing and numerical execution only; they do not demonstrate useful representations or astronomical generalization. The full smoke metadata is retained in \texttt{paper/experiments/learning-smoke-results.json}. The later end-to-end fixture additionally exercised multiscale target masking, the cosine target-encoder schedule, checkpoint loading, frozen-reference embedding, template search, evidence assembly, and finite-budget routing. It produced two shadow queue entries and explicitly set \texttt{qualifies\_reportability=false}. A separate malformed-training-input run exited at the JEPA stage and atomically retained a failed status rather than a completed marker.

\begin{table}[ht]
\centering
\caption{Evidence available in this paper and the conclusion each category supports.}
\label{tab:evidence-scope}
\begin{tabular}{p{0.20\linewidth}p{0.33\linewidth}p{0.35\linewidth}}
\toprule
Category & Available evidence & Supported interpretation \\
\midrule
Historical campaign & Surviving run and stage summaries & Workload and recorded dispositions; incomplete object-level history. \\
Software & Dated tests and deterministic replay & Behavior of tested invariants; no measured sky sensitivity. \\
Synthetic search & Frozen designs and per-trial statistics & Conditional recovery and noise-model failures; no survey selection function. \\
Live services & Offline parser and transport tests & Integration preparation; live operation remains unqualified here. \\
Real-sky comparison & Prospective protocol only & No comparative ranking or completeness claim. \\
\bottomrule
\end{tabular}
\end{table}

\input{experiments/search_section.tex}
...
The apparently large difference between 3,824 distinct retrieved objects and one detailed surviving candidate narrative should therefore not be called a false-positive rate. Most retrieved objects were never asserted to be positive examples, and rejected objects lack exhaustive truth labels. Likewise, there is no false-negative rate because the project did not revisit all rejected positions after a fixed maturation period to identify missed TNS transients.
...
No injection--recovery analysis was performed for the historical campaign, and rejected objects were not systematically re-evaluated after a fixed maturation interval. The separate synthetic diagnostic in Section~\ref{sec:synthetic-search} does not supply recovery estimates on real survey backgrounds. The campaign therefore provides no estimate of detection efficiency as a function of magnitude, cadence, host surface brightness, timescale, or transient class, and it does not measure the number of missed transients.
...
The band-aware feature path, content-addressed evidence, review binding, preflight gate, and shadow-model interfaces belong to \siderea\ and were not used in the historical campaign. An earlier audit environment included NumPy, pandas, scikit-learn, PyTorch, and Matplotlib but lacked the optional astronomy clients. Later installation of those clients does not by itself qualify a live integration; the dated environment and execution inventories distinguish availability from actual service use. The replay therefore verifies local deterministic behavior and blocking under missing evidence, not live broker or catalog integration. TS-JEPA, feature-space anomaly scoring, and the supervised baseline have not been compared with the heuristic on an object-grouped chronological cohort.

\section{Prospective Evaluation}
\label{sec:validation}

The decisive next question is whether a frozen SIDEREA policy yields more independently confirmed useful candidates at a fixed review budget than the incumbent ranking. A defensible comparison requires a complete eligible object cohort, an earlier development interval, an untouched later test interval, prediction-time information limits, and independently adjudicated outcomes after a fixed maturation delay. Unreviewed and unresolved sources must remain explicit rather than becoming negative labels.

The planned comparison should evaluate the heuristic, broker ranking, template search, supervised baseline and shadow embeddings on the same permitted inputs. It should report yield at the review budget, missed events where ascertainable, false alerts, latency and reviewer effort, with the uncertainty unit fixed before analysis. Component ablations and signal injection into representative survey backgrounds should identify which information contributes and where noise assumptions fail. Safety gates must remain enforced for operational decisions. These studies have not been completed; Appendix~\ref{sec:validation-protocol} records the detailed proposed protocol without treating it as a result.

\section{Conclusions}

We have presented a run-level audit of the \legacycampaign\ campaign and the implementation of its successor, \siderea. The campaign processed 3,824 distinct ALeRCE objects in 24 productive runs. Among the recorded downstream evaluations, 30 of 200 registry checks identified an existing TNS object and 55 of 193 catalog checks identified a known or suspected variable. The public TNS page independently verifies ZTF26abdqbqw = AT~2026rsp and report 312644; AT~2026pcz remains summary-attributed. Original campaign photometry, selection rows, and submission response bytes are not preserved, so neither claim is an independently reconstructed end-to-end discovery.
...
The source code, operating protocol, campaign-level audit record, manuscript source, bibliography, and figure-generation program are available at \url{https://github.com/build-the-future-11/IRIS-Space}. The reviewed research-alpha artifact is identified by tag \texttt{v0.3.0-research-alpha.3}; repository availability does not change the proprietary license declaration or grant permission to redistribute third-party records. Live-query outputs, cached light curves, candidate shortlists, report packages, and correspondence were intentionally untracked during the recorded campaign. Consequently, the aggregate results can be audited against the surviving summaries, but the exact stage-by-stage cohort cannot be independently reconstructed from the repository alone. Because upstream alert and catalog services evolve, future releases should archive immutable row-level manifests, service responses or content hashes where redistribution permits, configuration files, and final outcome tables.

\section*{Software}

The pipeline uses Python, NumPy \citep{harris2020}, pandas \citep{mckinney2010}, Astropy \citep{astropy2013,astropy2018}, astroquery \citep{ginsburg2019}, Matplotlib \citep{hunter2007}, and the ALeRCE Python client. Historical manuscript figures are reproducible from \texttt{paper/figures/make\_figures.py}; these parse the checked-in operations record and do not query live services. Synthetic diagnostic figures and trial statistics are produced by the separately archived runners in \texttt{paper/experiments}. The original and current machine-readable software audits are preserved in \texttt{paper/research/software-verification.json} and \texttt{paper/research/software-verification-2026-09-13.json}.
...
TS-JEPA mask fraction / scale jitter / seed & 0.50 / 0.50 / 2026 \\
TS-JEPA initial / final EMA momentum & 0.996 / 0.9999 \\
\bottomrule
\end{tabular}
\end{table}
