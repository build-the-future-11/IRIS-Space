...
We report the dated local verification snapshot archived in \texttt{paper/research/software-verification.json}, executed on 2026 September 8 using Python 3.11. That record binds source and test tree SHA-256 digests to the execution log; it is not a moving claim about every later repository revision. Table~\ref{tab:software-verification} distinguishes implementation evidence from scientific validation. That suite completed 405 tests without failure; strict static typing covered 74 source files, and branch-aware coverage was 78.35\%. Historical multi-interpreter results describe an earlier revision and are archived separately. These checks verify implemented invariants and failure behavior, not astronomical discovery performance.
