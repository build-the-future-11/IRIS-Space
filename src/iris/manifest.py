...
RUN_MANIFEST_SCHEMA = "iris.run_manifest.v1"
...
def _source_digest(root: Path, *, package_root: Path | None = None) -> str:
    ...
    A checkout is used only when its ``src/iris`` directory is the supplied
    ...
    checkout_package = root / "src" / "iris"
    ...
    if not candidates:
        ...
        if installed_root.is_dir():
            candidates.extend(
                (f"iris/{path.relative_to(installed_root).as_posix()}", path)
    ...
    if not candidates:
        raise RuntimeError("cannot identify any IRIS source files for the run manifest")
    digest = sha256()
    ...
    def create(
        ...
        *,
        ...
        return cls(
            ...
            environment={
                "iris-astronomy": _package_version("iris-astronomy"),
