...
from iris.evaluation import ranking_metrics
...
def save_baseline_bundle(path: str | Path, model: SupervisedBaseline) -> Path:
    ...
    try:
        ...
        manifest = {
            "format": "iris.supervised_baseline.v1",
...
def load_baseline_bundle(
    ...
    *,
    ...
    if manifest.get("format") != "iris.supervised_baseline.v1":
    ...
    if not isinstance(model, SupervisedBaseline):
        raise ValueError("bundle does not contain an IRIS SupervisedBaseline")
...
def _require_sklearn() -> tuple[Any, Any, Any, Any, str]:
    try:
        ...
        raise MissingOptionalDependency(
            "scikit-learn is required for iris.ml.baseline; install the ML optional dependencies"
        ) from exc
