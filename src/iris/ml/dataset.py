"""Light-curve tokenization and batching for IRIS representation learning.
...
PyTorch is an optional dependency for IRIS.  Importing this module works without
...
from iris.provenance import digest_value

try:  # Keep the non-ML IRIS install usable without a large optional dependency.
    import torch
...
TOKENIZATION_CONTRACT_VERSION = "iris.light_curve_tokens.v3"
DELTA_TIME_INDEX = 0
VALUE_INDEX = 1
ERROR_INDEX = 2
BAND_INDEX = 3
...
def require_torch() -> None:
    ...
    if torch is None:
        raise RuntimeError(
            "IRIS JEPA support requires PyTorch. Install the optional ML dependencies "
...
def batch_token_contract_digest(batch: Any) -> str | None:
    """Return one verified token-contract digest for an IRIS batch.
