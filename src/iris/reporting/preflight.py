...
from iris.integrity import verify_candidate_record, verify_candidate_version_payload
from iris.ledger import OutcomeLedger
from iris.manifest import RUN_MANIFEST_SCHEMA
from iris.provenance import CheckProvenance, digest_value
from iris.validation.binding import (
    EvidenceBindingContext,
    bind_completed_checks,
    preflight_digest,
)
from iris.validation.gates import evaluate_reportability
...
def _candidate_artifact_error(
    ...
    *,
    ...
    if (
        ...
        or evidence.get("schema") != "iris.candidates.v1"
...
def _publication_error(payload: Mapping[str, Any]) -> str | None:
    ...
    if (
        publication.get("schema") != "iris.run_publication.v1"
