...
These profiles are deliberately *not* operational policy.  The current SIDEREA
...
CAMPAIGNS: Mapping[str, CampaignProfile] = MappingProxyType(
    {
        "novel_transients": CampaignProfile(
            name="novel_transients",
...
def get_campaign(name: str) -> CampaignProfile:
    ...
    if normalized in {"i_spy", "ispy"}:
        normalized = "novel_transients"
    try:
