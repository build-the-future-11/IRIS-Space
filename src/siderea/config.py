"""Strict, typed TOML configuration for SIDEREA.
...
class ConfigError(ValueError):
    """Raised when an SIDEREA configuration file is missing or invalid."""
...
class GeneralConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.schema_version != 1:
            raise ConfigError("general.schema_version must be 1 for this SIDEREA release")
...
class ReviewConfig:
    ...
    trusted_assertion_key_ids: tuple[str, ...] = ()
    ...
    def __post_init__(self) -> None:
        ...
        _boolean_value(self.require_authenticated, "review.require_authenticated")
        if any(
            not isinstance(key, str)
            or len(key) != 64
            or any(character not in "0123456789abcdef" for character in key)
            for key in self.trusted_assertion_key_ids
        ):
            raise ConfigError("review.trusted_assertion_key_ids must contain SHA-256 digests")
        if self.require_authenticated and not self.trusted_assertion_key_ids:
            raise ConfigError("authenticated reviews require trusted_assertion_key_ids")
...
class JEPAConfig:
    ...
    dataset_path: Path = Path("var/siderea/datasets/jepa")
...
@dataclass(frozen=True, slots=True)
class SIDEREAConfig:
    """Complete validated configuration used to start an SIDEREA run."""
...
def default_config_path() -> Path:
    ...
    ``SIDEREA_CONFIG`` may override it for deployments, while CLI ``--config``
    remains the most explicit option.
    """

    configured = os.environ.get("SIDEREA_CONFIG")
    if configured:
...
def load_config(path: str | Path | None = None) -> SIDEREAConfig:
    """Load and validate an SIDEREA TOML file.
    ...
    general = GeneralConfig(
        name=_string(general_data, "name", "SIDEREA", "general"),
        campaign=_string(general_data, "campaign", "novel_transients", "general"),
    ...
    root = _path_value(
        _string(storage_data, "root", "../var/siderea", "storage"), base_dir, "storage.root"
    )
    storage = StorageConfig(
        root=root,
    ...
    network = NetworkConfig(
        ...
        user_agent=_string(network_data, "user_agent", "siderea-astronomy/0.3", "network"),
    )
    ...
    review = ReviewConfig(
        ...
        require_authenticated=_boolean(review_data, "require_authenticated", False, "review"),
        trusted_assertion_key_ids=_strings(review_data, "trusted_assertion_key_ids", (), "review"),
    )
...
def config_to_dict(config: SIDEREAConfig) -> dict[str, Any]:
