"""Deterministic local SIDEREA analysis pipeline.
...
from siderea.atomic import atomic_write_bytes, atomic_write_text
from siderea.config import SIDEREAConfig, config_to_dict
from siderea.data import create_snapshot
from siderea.domain import CandidateState
from siderea.evidence_context import serialized_verification_context
from siderea.features import compute_photometry_features, flatten_photometry_features
from siderea.ingest import IngestionBatch, ingest_csv, representative_coordinates
...
from siderea.validation.gates import GateDecision, GateOutcome, evaluate_reportability

PIPELINE_VERSION = "siderea.local_analysis.v4"
...
def _scientific_config(config: SIDEREAConfig) -> dict[str, Any]:
    ...
    manifest = RunManifest.create(
        {
            ...
            "siderea": config_payload,
            "ingestion": _json_value(batch.provenance),
        },
        command=["siderea", "analyze", batch.source],
        ...
        for candidate_id, group in frame.groupby("source_id", sort=True):
            ...
            provisional_payload = {
                ...
                "review_policy": {
                    ...
                    "require_authenticated": config.review.require_authenticated,
                    "trusted_assertion_key_ids": list(config.review.trusted_assertion_key_ids),
                },
