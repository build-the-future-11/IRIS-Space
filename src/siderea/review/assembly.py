...
from siderea.ranking import NightlyQueue, QueueCandidate, build_nightly_queue

from .dossier import write_candidate_dossier

REVIEW_SET_SCHEMA = "siderea.review_set.v1"
...
def _candidate_records(payload: Any) -> tuple[str, str, str, dict[str, dict[str, Any]]]:
    if not isinstance(payload, Mapping) or payload.get("schema") != "siderea.candidates.v1":
...
def assemble_review_set(
    ...
    *,
    ...
    identity_payload = {
        ...
        "queue": asdict(queue),
        "allocation_parameters": {"anomaly_threshold": anomaly_threshold},
        "selected_versions": {
...
def verify_review_set(directory: str | Path) -> dict[str, Any]:
    """Verify archived inputs, candidate versions, queue identity and every artifact.

    This establishes a portable selection record's integrity. It does not prove
    the upstream archive captured every astronomical arrival.
    """
    root = Path(directory).expanduser().resolve()
    manifest = json.loads((root / "manifest.json").read_text(encoding="utf-8"))
    if not isinstance(manifest, dict) or manifest.get("schema") != REVIEW_SET_SCHEMA:
        raise ValueError("unsupported review-set manifest")
    artifacts = manifest.get("artifacts")
    if not isinstance(artifacts, list) or not artifacts:
        raise ValueError("review-set artifact inventory is missing")
    seen: set[str] = set()
    for artifact in artifacts:
        if not isinstance(artifact, dict) or not isinstance(artifact.get("path"), str):
            raise ValueError("review-set artifact is invalid")
        relative = artifact["path"]
        path = (root / relative).resolve()
        if relative in seen or not path.is_relative_to(root) or not path.is_file():
            raise ValueError("review-set artifact path is duplicated, missing or outside bundle")
        seen.add(relative)
        if digest_file(path) != artifact.get("sha256") or path.stat().st_size != artifact.get(
            "size_bytes"
        ):
            raise ValueError("review-set artifact content differs")
    required = {"inputs/ranked_candidates.csv", "inputs/candidates.json", "queue.json"}
    if not required <= seen:
        raise ValueError("review-set lacks required archived inputs")
    ranking_bytes = (root / "inputs/ranked_candidates.csv").read_bytes()
    candidate_bytes = (root / "inputs/candidates.json").read_bytes()
    run_id, fingerprint, pipeline_version, records = _candidate_records(json.loads(candidate_bytes))
    frame = pd.read_csv(
        BytesIO(ranking_bytes), dtype={"candidate_id": "string", "candidate_version": "string"}
    )
    _validate_ranking_binding(
        frame, records, scientific_fingerprint=fingerprint, pipeline_version=pipeline_version
    )
    queue = manifest.get("queue")
    if not isinstance(queue, dict) or not isinstance(queue.get("entries"), list):
        raise ValueError("review-set queue is invalid")
    allocation = manifest.get("allocation_parameters")
    if not isinstance(allocation, dict) or set(allocation) != {"anomaly_threshold"}:
        raise ValueError("review-set lacks allocation parameters; generate a current review set")
    try:
        rebuilt = asdict(
            build_nightly_queue(
                _queue_candidates_from_frame(
                    frame, eligibility_policy=manifest["eligibility_policy"]
                ),
                budget=queue["budget"],
                anomaly_slots=queue["requested_anomaly_slots"],
                anomaly_threshold=allocation["anomaly_threshold"],
                audit_slots=queue["requested_audit_slots"],
                audit_seed=queue["audit_seed"],
                selection_policy=f"{manifest['eligibility_policy']}_v1",
            )
        )
        # JSON arrays correspond to dataclass tuples in the allocator.
        if json.loads(json.dumps(rebuilt)) != queue:
            raise ValueError("review-set queue differs from deterministic allocation")
    except (KeyError, TypeError) as exc:
        raise ValueError("review-set allocation parameters are invalid") from exc
    selected = [entry["candidate_id"] for entry in queue["entries"]]
    if len(set(selected)) != len(selected) or not set(selected) <= set(records):
        raise ValueError("review-set selection does not match candidate evidence")
    identity = {
        "schema": REVIEW_SET_SCHEMA,
        "source_run_id": run_id,
        "source_scientific_fingerprint": fingerprint,
        "source_pipeline_version": pipeline_version,
        "allocation_parameters": allocation,
        "ranking_sha256": sha256(ranking_bytes).hexdigest(),
        "candidates_sha256": sha256(candidate_bytes).hexdigest(),
        "queue": queue,
        "selected_versions": {
            identifier: records[identifier]["candidate_version"] for identifier in selected
        },
    }
    if any(manifest.get(key) != value for key, value in identity.items()):
        raise ValueError("review-set manifest differs from archived input bindings")
    if manifest.get("review_set_id") != f"review-set-{digest_value(identity)[:20]}":
        raise ValueError("review-set identity digest differs")
    actual_queue = json.loads((root / "queue.json").read_text(encoding="utf-8"))
    expected_queue = {
        **queue,
        "review_set_id": manifest["review_set_id"],
        "source_run_id": run_id,
        "ranking_sha256": identity["ranking_sha256"],
        "candidates_sha256": identity["candidates_sha256"],
        "safety_boundary": "review selection only; never reporting authorization",
    }
    if actual_queue != expected_queue or manifest.get("selected_candidate_ids") != selected:
        raise ValueError("review-set queue differs from manifest")
    return manifest
