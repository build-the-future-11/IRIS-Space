...
def test_semantic_trial_streams_are_loop_order_independent() -> None:
    ...
    forward = {
        key: trial_rng(DEVELOPMENT_SEED, key).integers(0, 2**31) for key in keys
    }
    reverse = {
        key: trial_rng(DEVELOPMENT_SEED, key).integers(0, 2**31)
...
def test_noncanonical_or_invalid_keys_fail_closed() -> None:
    ...
    with pytest.raises(ValueError, match="canonical"):
        trial_seed_words(
            DEVELOPMENT_SEED,
            '{"schema":"siderea.robust_search_trial_rng.v2"}',
        )
